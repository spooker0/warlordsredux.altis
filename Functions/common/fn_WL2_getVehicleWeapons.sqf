...
private _turretPaths = _baseVehicle call BIS_fnc_vehicleCrewTurrets;
{
    ...
    private _turretDisplay = format ["<t color='#efbf04' shadow='0' size='1.05'>%1</t> <t color='#2255dd' shadow='0' size='1.05'>(%2)</t>%3", _turretName, _turretWeaponsDisplay, _turretMagazineDisplay];
...
if (count _pylonsInfo != 0) then {
    private _pylonNames = _pylonsInfo apply {
        ...
        if (_pylonAttachment == "") then {
            ...
            [_pylonAttachment] call BIS_fnc_WL2_getMagazineName;
        };
    };
    ...
    private _pylonDisplay = format ["<t color='#efbf04' shadow='0' size='1.05'>Pylon</t><br/><t color='#2255dd' shadow='0'>%1</t>", _pylonNameDisplay];
